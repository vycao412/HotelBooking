...
public class CancelController extends HttpServlet {
    ...
    private static final String SUCCESS = "TrackBookingController";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        LOG.info("CancelController");

        try {
            HttpSession session = request.getSession();
            String bookingID = request.getParameter("id");
            List<CartDTO> history = (List<CartDTO>) session.getAttribute("HISTORY");
            for (CartDTO cart : history) {
                if (cart.getId().equals(bookingID)) {
                    cart.remove(bookingID);
                    break;
                }
                RoomDAO roomDao = new RoomDAO();
                for (RoomDTO room : cart.getCart().values()) {
                    roomDao.updateQuanCancel(room);
                }
            }
            session.setAttribute("HISTORY", history);
            CartDAO dao = new CartDAO();
            
            dao.updateStatus(6, bookingID);
            url = SUCCESS;

        } catch (Exception e) {
            LOG.error("Error at CancelController: " + e.toString());
        } finally {
